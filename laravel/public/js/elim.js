...
for (let i = 0; i < boton_elim.length; i++) {
    ...
    boton_elim[i].addEventListener('click',(e)=>{
        ...
        token.setAttribute('value',boton_elim[i].parentNode[0].value)
        //console.log(boton_elim[i].parentNode[0].value);
        //<input type="hidden" name="_token" value="g4Y9Leizo4LofCnO0rgDg6pamB3LUzuOkOSmtzPY">
        //<input type="hidden" name="_token" value="pnsVV2sfsn0TUOE0wwBlrwovSyaZQpT6EIbBFiHv">
...
if (despl[x].style.height == '0px') {
    ...
    despl[x].classList.toggle('holaa')
}else{
    despl[x].classList.toggle('holaa')
    // despl[x].style.height = '0px'
}
